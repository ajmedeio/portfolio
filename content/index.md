...
You'll find three main ways to navigate. First, use the `Tree View` by toggling the "hamburger" icon in the top-left. Second, use the `Graph View` located on the bottom of each page. Third, use the `Chronological View` located below the title of each page — it depicts each entry on a timeline with the current page highlighted. The `Tree View` is how the underlying files are organized and is a decent starting point, the `Graph View` provides some spatial awareness to each page and how it may connect to others, and the `Chronological View` places everything in temporal context.
